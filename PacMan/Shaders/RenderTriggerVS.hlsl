...
StructuredBuffer<int> g_InstanceStateBuffer : register(t1);
...
OutputData Main(InputData inputData)
{
	...
	int instanceState = g_InstanceStateBuffer[inputData.instanceId];
	if (instanceState == TRIGGER_STATE_ACTIVE)
	{
	...
	{
		outputData.clipSpacePos = float4(-1.0f, -1.0f, -1.0f, 1.0f);
		outputData.texCoords = float2(0.0f, 0.0f);
	}
